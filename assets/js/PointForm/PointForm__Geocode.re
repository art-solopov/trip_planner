...
let resultsListener = (form: Dom.htmlFormElement, event: Dom.event_like('a)) => {
  ...
  if (target->classList->ClassList.contains("gc-save")) {
    let parent = target->closest(".geocode-result")->Belt.Option.getExn;
...
let default = () => {
  let results =
    document->getElementById("geocode_results")->Belt.Option.getExn;
  let form = document->getElementByIdForm("point_form")->Belt.Option.getExn;
  let button =
    document->getElementByIdInput("btn_geocode")->Belt.Option.getExn;
